...
# 1. Configuration - DO NOT CHANGE THESE
...
def main():
    ...
    # [START] tag must be the first line
    print(f"[START] task=validation env=openenv model={MODEL_NAME}", flush=True)

    # Retry loop to handle the "Connection Error" you are seeing
    for attempt in range(5):
        try:
            ...
            success = True
            break  # Stop retrying once we succeed

        except Exception as e:
            if attempt < 4:
                # Log the retry internally without breaking the tag format
                print(f"Connection busy. Retrying {attempt + 1}/5...", flush=True)
                time.sleep(10)
            else:
                # Final failure log
                rewards.append(0.00)
...
if __name__ == "__main__":
    main()
    # Keep the container running so the Scaler Dashboard sees "Running" (Green)
